...
LDFLAGS_BUILD = -ldflags "-X $(SERVICE_PATH).BuildTime=$(BUILD_TIME) -X $(SERVICE_PATH).GitCommit=$(GIT_COMMIT) -X $(SERVICE_PATH).Version=$(VERSION) -X github.com/ONSdigital/dp-frontend-feedback-controller/config.RendererVersion=$(APP_RENDERER_VERSION)"
...
.PHONY: build
build: generate-prod
	go build -tags 'production' $(LDFLAGS_BUILD) -o $(BINPATH)/dp-frontend-feedback-controller

.PHONY: lint
lint: generate-prod ## Run linters against Go code
	golangci-lint run ./... --build-tags 'production'

.PHONY: debug
debug: generate-debug
	go build -tags 'debug' $(LDFLAGS_DEBUG) -o $(BINPATH)/dp-frontend-feedback-controller
